...
def reorder_memories(container: Any, session_id: str, strategy: str = "importance") -> int:
    ...
    try:
        ...
        logger.info(f"[OK] Reordered {count} memories for session {session_id} using {strategy}")
        return count

    except Exception as e:
        logger.error(f"[ERROR] Failed to reorder memories for session {session_id}: {e}")
        return 0
